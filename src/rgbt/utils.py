...
def ltwh_2_ltrb(rect:list) -> list:
    return [rect[0], rect[1], rect[2]+rect[0]-1, rect[3]+rect[1]-1]


def ltrb_2_ltwh(rect:list) -> list:
    if _strict:
        return [rect[0], rect[1], rect[2]-rect[0]+1, rect[3]-rect[1]+1]
    else:
...
def xywh_2_ltrb(rect:list) -> list:
    """
...
def xywh_2_ltwh(rect:list) -> list:
    """
...
def corner_2_ltrb(rect:list) -> list:
    return [rect[0], rect[1], rect[-2-2], rect[-1-2]]


def corner_2_ltwh(rect:list) -> list:
    if _strict:
...
def bbox_type_trans(bbox_type_src:str, bbox_type_new:str):
...
def normalize(cx, cy, gt_w, gt_h, eps=1e-8) -> tuple[float, float]:
    return cx/(gt_w+eps), cy/(gt_h+eps)


def CLE(rect1:list, rect2:list) -> float:
...
def normalize_CLE(rect1:list, rect2:list) -> float:
    """ caculate center location error
    NOTE: Default rect2 is groundtruth
    Args:
        rect1: (l, t, w, h)
...
def IoU(rect1:list, rect2:list) -> float:
